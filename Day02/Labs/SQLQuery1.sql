...
/* Build Your Own Executive Dashboard */
CREATE OR ALTER PROCEDURE usp_ExecutiveDashboard
AS
BEGIN
	SET NOCOUNT ON;

	SELECT COUNT(p.PatientId) AS TotalActivePatients
	FROM Patient p
	WHERE p.IsActive = 1;

	SELECT d.Name AS Department, COUNT(e.EncounterId) As TotalEncounters,
	RANK()
	OVER(
		ORDER BY COUNT(e.EncounterID) DESC
	) AS Ranking
	...
	SELECT e.EncounterId, d.Name AS Department,
    AVG(DATEDIFF(DAY, e.AdmitDate, e.DischargeDate))
    OVER (
        PARTITION BY e.DepartmentId
    ) AS DepartmentAverageLOS
	FROM Encounter e
